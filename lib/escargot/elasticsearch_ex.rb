module Escargot
  ...
  module AdminIndexVersions
    ...
    def create_index_version(index, create_options)
      ...
      Escargot.elastic_search_client.create_index(index_with_timestamp, create_options)
    ...
    def current_index_version(index)
      Escargot.elastic_search_client.index_status(index)["indices"].keys.first rescue nil
    end
    
    # "deploys" a new version as the current one
    def deploy_index_version(index, new_version)
      Escargot.elastic_search_client.refresh(new_version)
      if current_version = current_index_version(index)
        Escargot.elastic_search_client.alias_index(
          :add => {new_version => index}, 
          :remove => {current_version => index}
        )
      else
        Escargot.elastic_search_client.alias_index(:add => {new_version => index})
      end
    end
    ...
    def prune_index_versions(index)
      ...
      old_versions.each do |version|
        Escargot.elastic_search_client.delete_index(version)
      end
    end

    # lists all current, old, an in-progress versions for this index
    def index_versions(index)
      Escargot.elastic_search_client.index_status()["indices"].keys.grep(/^#{index}_/)
    end
